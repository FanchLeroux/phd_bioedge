...
dirc_data = pathlib.Path(
    __file__).parent.parent.parent.parent.parent.parent / "data"
...
def acquire(cam, n_frames, exp_time, roi=False, dirc=False, overwrite=False):
    ...
    if roi != False:
        image = image[:, roi[1]-roi[3]//2:roi[1] +
                      roi[3]//2, roi[0]-roi[2]//2:roi[0]+roi[2]//2]
...
def live_view(get_frame_func, cam, roi, dirc=False, overwrite=True, interval=0.005):
    ...
    frame = get_frame_func(cam, cam.n_frames, cam.exp_time,
                           roi=roi, dirc=False, overwrite=True)[0, :, :,]
    ...
    while plt.fignum_exists(fig.number):  # Loop while window is open
        frame = get_frame_func(
            cam, cam.n_frames, cam.exp_time, roi=roi, dirc=False, overwrite=True)[0, :, :,]
        im.set_data(frame)
        # Adjust color scale
...
OutputPulseImageRefresh = ct.c_uint(0)
...
slm_lib.Load_LUT_file(board_number, str(
    dirc_data / "slm" / "LUT" / "12bit_linear.lut").encode('utf-8'))
slm_flat = np.asarray(Image.open(
    str(dirc_data / "slm" / "WFC" / "1920black.bmp")))
...
slm_lib.Load_LUT_file(board_number, str(
    dirc_data / "slm" / "LUT" / "utc_2025-06-27_11-37-41_slm0_at675.lut").encode('utf-8'))
...
tilt_angle = 45.0  # [deg]
tilt = get_tilt([1152, 1920], theta=np.deg2rad(tilt_angle),
...
command = display_phase_on_slm(tilt_in_pupil, slm_flat, slm_shape=[
                               1152, 1920], return_command_vector=True)
plt.figure()
plt.imshow(np.reshape(command, slm_shape))
plt.title("Command")

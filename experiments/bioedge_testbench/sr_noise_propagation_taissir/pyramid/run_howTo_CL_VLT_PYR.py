# -*- coding: utf-8 -*-
"""
Created on Thu Nov 16 17:07:23 2023
...
pyr_20 = Pyramid(nSubap             = param['nSubaperture'],\
              telescope             = tel,\
...
pyr_20_GSR = Pyramid(nSubap                = param['nSubaperture'],\
              telescope             = tel,\
              modulation            = param['modulation'],\
              lightRatio            = param['lightThreshold'],\
              n_pix_separation      = param['n_pix_separation'      ],\
...
pyr_40 = Pyramid(nSubap                = 2*param['nSubaperture'],\
              telescope             = tel,\
              modulation            = param['modulation'],\
              lightRatio            = param['lightThreshold'],\
              n_pix_separation      = param['n_pix_separation'      ],\
...
x = 0.25
#pyr_20_GSR.apply_shift_wfs(sx=[-x,x,x,x],sy = [x,x,-x,x] )
pyr_20_GSR.apply_shift_wfs(sx=[x,-x,-x,x],sy = [-x,-x,x,x] )

plt.figure()
plt.imshow(pyr_20_GSR.cam.frame-pyr_20.cam.frame)
...
display_wfs_signals(pyr_20,pyr_20.referenceSignal)
plt.colorbar()
display_wfs_signals(pyr_20,pyr_20_GSR.referenceSignal)
plt.colorbar()
...
calib_pyr_20 = InteractionMatrix(  ngs            = ngs,\
                            atm            = atm,\
                            tel            = tel,\
                            dm             = dm,\
                            wfs            = pyr_20,\
                            ...
                            stroke         = stroke,\
                            nMeasurements  = 2,\
                            noise          = 'off',single_pass=False,display=True)
    
calib_pyr_20_GSR = InteractionMatrix(  ngs            = ngs,\
                                atm            = atm,\
                                tel            = tel,\
                                dm             = dm,\
                                wfs            = pyr_20_GSR,\
...
calib_pyr_40 = InteractionMatrix(  ngs            = ngs,\
                                atm            = atm,\
                                tel            = tel,\
                                dm             = dm,\
                                wfs            = pyr_40,\
...
test_wfs = [pyr_40,pyr_20,pyr_20_GSR]


test_calib = [calib_pyr_40,calib_pyr_20,calib_pyr_20_GSR]
plt.close('all')

title_fig = ['40x40','20x20','20x20 + GSR'] 

for i_mode in [200,300,400,500,600, 700, 800]:
    propa = []

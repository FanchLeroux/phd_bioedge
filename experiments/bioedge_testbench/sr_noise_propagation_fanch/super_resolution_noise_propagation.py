...
n_modes_list = np.linspace( int(0.5*(np.pi*(n_subaperture/2)**2)), int(np.pi * n_subaperture**2), num=5, dtype=int)
...
sx = np.array(sx)+0*0.25
sy = np.array(sy)+0*0.25

sx = 0.5*sx
sy = 0.5*sy

wfs_sr = BioEdge(nSubap = n_subaperture, 
              telescope = tel, 
              modulation = 0.,
              grey_width = grey_width, 
              lightRatio = 0.5,
...
# 
# binning_factor = wfs_sr.bioFrame.shape[0]/wfs_sr.cam.frame.shape[0]
...
# wfs_sr.mask = mask_sr_cam_frame
# =============================================================================

wfs_sr.apply_shift_wfs(sx, sy, units='pixels')
wfs_sr.modulation = 0. # update reference intensities etc.

tel*wfs_sr
wfs_sr.wfs_measure(tel.pupil)
flat_frame_sr = wfs_sr.cam.frame

#%% Callibration - SR

tel.resetOPD()
calib_sr = InteractionMatrix(ngs, atm, tel, dm, wfs_sr, M2C = M2C, stroke = stroke, single_pass=False)

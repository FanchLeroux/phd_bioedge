# -*- coding: utf-8 -*-
"""
Created on Mon May  5 16:18:12 2025
...
total, residual_1, strehl, dm_coefs, turbulence_phase_screens,\
    ...
    close_the_loop_delay(tel, ngs, atm, dm, gbioedge, reconstructor_gbioedge, 
                       ...
                       delay=1, photon_noise=True, seed=seed, save_telemetry=True, save_psf=True)
...
total, residual_2, strehl, dm_coefs, turbulence_phase_screens,\
    ...
    close_the_loop_delay(tel, ngs, atm, dm, gbioedge, reconstructor_gbioedge, 
                         param['loop_gain'], param['n_iter'], 
                       delay=2, photon_noise=True, seed=seed, save_telemetry=True, save_psf=True)
    
total, residual_3, strehl, dm_coefs, turbulence_phase_screens,\
    residual_phase_screens, wfs_frames, short_exposure_psf, buffer_wfs_measure =\
    close_the_loop_delay(tel, ngs, atm, dm, gbioedge, reconstructor_gbioedge, 
                         param['loop_gain'], param['n_iter'], 
                       delay=3, photon_noise=True, seed=seed, save_telemetry=True, save_psf=True)
...
total, residual, strehl, dm_coefs, turbulence_phase_screens,\
    residual_phase_screens, wfs_frames, short_exposure_psf, buffer_wfs_measure =\
    close_the_loop_delay(tel, ngs, atm, dm, sgbioedge_sr, reconstructor_sgbioedge_sr, 
                         param['loop_gain'], param['n_iter'], 
                       delay=3, photon_noise=True, seed=0, save_telemetry=True, save_psf=True)

plt.figure()
plt.plot(residual)

plt.figure()
plt.imshow(np.log(short_exposure_psf[:,:,50:].sum(axis=-1)))

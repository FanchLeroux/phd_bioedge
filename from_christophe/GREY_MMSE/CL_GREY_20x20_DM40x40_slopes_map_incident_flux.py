...
reconstructed_modes_MMSE = np.asarray(REC_MMSE @ bio_20.bioSignal * TO_M0) / stroke_test
...
axs[2].plot(reconstructed_modes_MMSE[n_mode,:n_mode+20])

plt.figure()
plt.plot(np.diag(reconstructed_modes_MMSE)[50:250])


    

#%% Reconstruct 1 mode with MMSSE reconstructor

n_mode = 50

plt.figure()
plt.plot(reconstructed_modes_MMSE[n_mode,:n_mode+20])
...
#%% Reconstruct 1 mode with LSE reconstructor

n_mode = 50

reconstructed_modes_LSE = np.asarray(reconstructor_LSE @ bio_20.bioSignal) / stroke_test

plt.figure()
plt.plot(reconstructed_modes_LSE[n_mode,:n_mode+20])
